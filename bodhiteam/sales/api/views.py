...
class GetMyNewLeads(APIView):
    def get(self, request):
        leads = Lead.objects.filter(assignedTo=self.request.user.salesexecutive,
                                    lead_status='is_successfull_lead').order_by('-date')
...
class GetMyNewLeadsDateWise(APIView):
    def post(self, request, *args, **kwargs):
        ...
        data = request.data
        date = data['date']
        date_wise_leads = Lead.objects.filter(
            assignedTo=my_profile, date__date=date, lead_status='is_successfull_lead').order_by('-id')
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            try:
                ...
                nextCallDate = datetime.datetime.strptime(
                    nextCallDate, '%Y-%m-%d %H:%M')
            except:
                nextCallDate = None
            try:
                demoDate = demoDate.strip('"')
                demoDate = datetime.datetime.strptime(
                    demoDate, '%Y-%m-%d %H:%M')
            except:
                demoDate = None
...
class GetWorkedLeadsApi(APIView):
    def get(self, request):
        leads = Lead.objects.filter(
            assignedTo=self.request.user.salesexecutive, lead_status='worked_lead').order_by('-date')
        serializer = LeadSerializer(leads, many=True)
        return Response({'Leads': serializer.data})
...
class GetFeedbackOrDemosSpecificLeadWiseApi(APIView):
    def post(self, request, *args, **kwargs):
        ...
        context = {'feedbacks': feedbackserializer.data,
                   'demosfeedbackes': demosserializer.data}
        return Response(context)
...
class GetMyAssignedLeadsAPI(APIView):
    def get(self, request):
        assignedLeadss = Lead.objects.filter(Q(Q(Q(feeback_lead__nextCall=request.user.salesexecutive) | Q(demo_lead__demo_nextCall=request.user.salesexecutive)) & Q(Q(lead_status='worked_lead') | Q(
            lead_status='is_successfull_lead'))) & ~Q(Q(feeback_lead__by=request.user.salesexecutive) | Q(demo_lead__by=request.user.salesexecutive))).order_by('-feeback_lead__time')
...
class GetSpecificLeadAndFeedbackAPI(APIView):
    def post(self, request):
        ...
        try:
            ...
            context = {'lead': leadserializer.data,
                       'feedback': feedbackserializer.data}
            return Response(context)
...
class SendMessageToUserAPI(APIView):
    def get(self, request):
        ...
        for i in users:
            if i.lead_assign.filter(assignedTo=self.request.user.salesexecutive).exists():
                leads = i.lead_assign.filter(
                    assignedTo=self.request.user.salesexecutive).order_by('-date')
                break
        serializer = LeadSerializer(leads, many=True)
    ...
    def post(self, request):
        ...
        Massages.objects.create(senderId=self.request.user.salesexecutive, reciverId=SalesExecutive.objects.get(
            id=data['reciver']), lead=lead_id, feedback=Lead_Feedback_Id, massage=data['message'], datetime=datetime.datetime.now())

        get, create = Notification.objects.get_or_create(notification_user=SalesExecutive.objects.get(
            id=data['reciver']), sender_user=self.request.user.salesexecutive)
        get.massage = self.request.user.salesexecutive.name + ' ' + \
            self.request.user.salesexecutive.typeExecutive + " send a massage"
...
class GetFeedbackesLeadWiseUsingAjexAPI(APIView):
    def get(self, request):
        feedbackes = FeedBack.objects.filter(
            lead=request.data['Selected_Lead_id']).order_by('-typeFeedBack')
        feedbackserializer = FeedBackSerializer(feedbackes, many=True)
...
class MessagesInboxAPI(APIView):
    def get(self, request):
        Allmassages = Massages.objects.filter(
            reciverId=self.request.user.salesexecutive).order_by('-datetime')
        unread_message_length = Allmassages.filter(massagRead=False).count()
        Notification.objects.filter(
            notification_user=self.request.user.salesexecutive).delete()
        messageserializer = MassagesSerializer(Allmassages, many=True)
        context = {'UserAllMessages': messageserializer.data,
                   'unread_message_length': unread_message_length}
        return Response(context)


class GetMySpecificMessageAPI(APIView):
    def post(self, request):
        specific_user_massages = Massages.objects.filter(
            reciverId=self.request.user.salesexecutive, senderId=request.data['user_id']).order_by('-datetime')
        specific_user_massages.update(massagRead=True)
        messageserializer = MassagesSerializer(
            specific_user_massages, many=True)
        return Response({'messgaes': messageserializer.data})


class SpecificUserNotificationsAPI(APIView):
    def get(self, request):
        myallnotifications = DemoFeedback_And_LeadFeedback_Notifications.objects.filter(
            notification_user=self.request.user.salesexecutive).order_by('-nextDate')
        serializer = DemoFeedback_And_LeadFeedback_NotificationsSerializer(
            myallnotifications, many=True)
...
class TodayScheduleReminderAPI(APIView):
    def get(self, request):
        ...
        myallSchedule = DemoFeedback_And_LeadFeedback_Notifications.objects.filter(
            notification_user=me).order_by('-nextDate')
        counter = 0
        dlfns_list = []
        try:
            for i in myallSchedule:
                if str(i.nextDate).split(' ')[0] == str(datetime.now()).split(' ')[0]:
                    ...
                    dlfn_dict = {'assignedBy': assignedBy, 'lead': i.lead.personName,
                                 'notification_type': i.notification_type, 'massage': i.massage}
                    dlfns_list.append(dlfn_dict)
            context = {'todayTotalSchedule': counter,
                       'TodaySchedule': dlfns_list}
        except Exception as e:
...
class AddSuccessfullyLeadsAPI(APIView):
    def get(self, request):
        leads = Lead.objects.filter(Q(Q(assignedTo=self.request.user.salesexecutive) | Q(feeback_lead__nextCall=self.request.user.salesexecutive)) & Q(
            Q(lead_status='is_successfull_lead') | Q(lead_status='worked_lead'))).distinct()
    ...
    def post(self, request):
        ...
        lead.save()
        SuccessfullyLead.objects.create(by=self.request.user.salesexecutive, lead=lead,
                                        priceQuoted=data['decidedPrice'], extra_requirement=data['extrarequirments'], datetime=datetime.datetime.now())
        return Response('Successfully added')


class SpecificPersonSuccessfullyLeadsAPI(APIView):
    def get(self, request):
        successfullyleads = SuccessfullyLead.objects.filter(
            by=self.request.user.salesexecutive).order_by('datetime')
        successfullLeadsSerializer = SuccessfullyLeadSerializer(
            successfullyleads, many=True)
    ...
    def post(self, request):
        ...
        successfullyleads = SuccessfullyLead.objects.filter(
            by=self.request.user.salesexecutive, lead__date__gte=from_date, lead__date__lte=to_date).order_by('datetime')
        successfullLeadsSerializer = SuccessfullyLeadSerializer(
            successfullyleads, many=True)
...
class ApplyFilterAndSeacrhAPI(APIView):
    ...
    def post(self, request):
        ...
        if searching_value:
            leads = Lead.objects.filter(Q(Q(Q(personName__icontains=searching_value) | Q(contactPhone__icontains=searching_value) | Q(email__icontains=searching_value)) & Q(Q(assignedTo=self.request.user.salesexecutive) | Q(
                feeback_lead__nextCall=self.request.user.salesexecutive))) & Q(Q(lead_status='worked_lead') | Q(lead_status='is_successfull_lead'))).order_by('-date').filter(date__gte=last_month).distinct()
        else:
            ...
            if from_date and selectedvale != 'None':
                ...
                leads = Lead.objects.filter(Q(Q(Q(Q(feeback_lead__feedback__in=selectedvale) | Q(demo_lead__demo_feedback__in=selectedvale) | Q(feeback_lead__rating__in=selectedrating) | Q(demo_lead__demo_rating__in=selectedrating)) & (Q(assignedTo=self.request.user.salesexecutive) | Q(
                    feeback_lead__nextCall=self.request.user.salesexecutive) | Q(demo_lead__demo_nextCall=self.request.user.salesexecutive))) & Q(Q(lead_status='worked_lead') | Q(lead_status='is_successfull_lead'))) & Q(date__gte=from_date, date__lte=To_data)).order_by('-date').distinct()
            elif from_date and data['selected_rating']:
                leads = Lead.objects.filter(Q(Q(Q(Q(feeback_lead__rating__in=selectedrating) | Q(demo_lead__demo_rating__in=selectedrating)) & Q(Q(assignedTo=self.request.user.salesexecutive) | Q(feeback_lead__nextCall=self.request.user.salesexecutive) | Q(
                    demo_lead__demo_nextCall=self.request.user.salesexecutive))) & Q(Q(lead_status='worked_lead') | Q(lead_status='is_successfull_lead'))) & Q(date__gte=from_date, date__lte=To_data)).distinct()
            elif from_date:
                leads = Lead.objects.filter(Q(Q(date__gte=from_date, date__lte=To_data) & Q(Q(assignedTo=self.request.user.salesexecutive) | Q(feeback_lead__nextCall=self.request.user.salesexecutive) | Q(
                    demo_lead__demo_nextCall=self.request.user.salesexecutive))) & Q(Q(lead_status='worked_lead') | Q(lead_status='is_successfull_lead'))).order_by('-date').distinct()
            else:
                selectedvale = selectedvale.strip('][').split(',')
                leads = Lead.objects.filter(Q(Q(Q(Q(feeback_lead__feedback__in=selectedvale) | Q(demo_lead__demo_feedback__in=selectedvale) | Q(feeback_lead__rating__in=selectedrating) | Q(demo_lead__demo_rating__in=selectedrating)) & (Q(assignedTo=self.request.user.salesexecutive) | Q(
                    feeback_lead__nextCall=self.request.user.salesexecutive) | Q(demo_lead__demo_nextCall=self.request.user.salesexecutive))) & Q(Q(lead_status='worked_lead') | Q(lead_status='is_successfull_lead'))) & Q(date__gte=last_month)).order_by('-date').distinct()
...
class SortingApplyAPI(APIView):
    def post(self, request):
        ...
        if sorting_type == 'latest':
            leads = Lead.objects.filter(Q(Q(assignedTo=self.request.user.salesexecutive) | Q(feeback_lead__nextCall=self.request.user.salesexecutive)) & Q(
                Q(lead_status='worked_lead') | Q(lead_status='is_successfull_lead'))).order_by('-date').distinct()
        else:
            leads = Lead.objects.filter(Q(Q(assignedTo=self.request.user.salesexecutive) | Q(feeback_lead__nextCall=self.request.user.salesexecutive)) & Q(
                Q(lead_status='worked_lead') | Q(lead_status='is_successfull_lead'))).order_by('date').distinct()
        leadsSerializer = LeadSerializer(leads, many=True)
...
class SalesUserProfileApi(APIView):
    ...
    def post(self, request):
        data = request.data
        currentUser = SalesExecutive.objects.get(
            id=self.request.user.salesexecutive.id)
...
class FilterAndSortForAdminApi(APIView):
    ...
    def post(self, request):
        ...
        selected_feedback_for_filter = selected_feedback_for_filter.strip(
            '][').split(',')

        if data['selected_feedback'] and selected_fromDate_for_filter:
            Filterd_leads = Lead.objects.filter(Q(Q(feeback_lead__feedback__in=selected_feedback_for_filter) | Q(demo_lead__demo_feedback__in=selected_feedback_for_filter)) & Q(
                date__date__gte=selected_fromDate_for_filter, date__date__lte=selected_toDate_for_filter)).order_by('-date').distinct()

        elif selected_fromDate_for_filter:
            Filterd_leads = Lead.objects.filter(
                date__date__gte=selected_fromDate_for_filter, date__date__lte=selected_toDate_for_filter).order_by('-date')
        else:
            Filterd_leads = Lead.objects.filter(Q(feeback_lead__feedback__in=selected_feedback_for_filter) | Q(
                demo_lead__demo_feedback__in=selected_feedback_for_filter)).order_by('-date').distinct()
...
class UserGetWorkedLeadsFileterWiseAPI(APIView):
    def post(self, request):
        ...
        if getDateWise:
            leads = Lead.objects.filter(
                date__date=getDateWise, assignedTo=my_profile, lead_status='worked_lead').order_by('-id').distinct()

        elif getFeedbackWise:
            leads = Lead.objects.filter(Q(feeback_lead__feedback=getFeedbackWise) | Q(
                demo_lead__demo_feedback=getFeedbackWise), assignedTo=my_profile, lead_status='worked_lead').order_by('-id').distinct()

        elif getRatingWise:
            leads = Lead.objects.filter(Q(feeback_lead__rating=getRatingWise) | Q(
                demo_lead__demo_rating=getRatingWise), assignedTo=my_profile, lead_status='worked_lead').order_by('-id').distinct()

        else:
            leads = Lead.objects.filter(
                feeback_lead__demo=True, assignedTo=my_profile, lead_status='worked_lead').order_by('-id').distinct()
        ...
        for lead in leads:
            ...
            lead_dict = {'id': lead.id, 'personName': lead.personName, 'instituteName': lead.instituteName, 'contactPhone': lead.contactPhone, 'email': lead.email, 'numberStudents': lead.numberStudents,
...
class GetMyTodayAssignedLeadsAPI(APIView):
    def get(self, request):
        ...
        assignedLeadss = Lead.objects.filter(Q(Q(feeback_lead__nextCall=me) | Q(demo_lead__demo_nextCall=me)) & Q(
            Q(feeback_lead__nextCallDate__date=today_date) | Q(demo_lead__demo_nextCallDate__date=today_date))).order_by('-feeback_lead__time')
        serializer = LeadSerializer(assignedLeadss, many=True)
...
class GetMyOldLeadsAPI(APIView):
    def get(self, request):
        leads = Lead.objects.filter(assignedTo=self.request.user.salesexecutive,
                                    lead_status='old_lead', is_this_old_lead=True).order_by('-date')
        serializer = LeadSerializer(leads, many=True)
        return Response({'Leads': serializer.data})
    ...
    def post(self, request):
        ...
        if leaddate:
            old_leads = Lead.objects.filter(
                assignedTo=profile, is_this_old_lead=True, date__date=leaddate)
            ...
            dfeedback = DemoFeedback.objects.filter(
                demo_feedback=feedback, by=profile)
        ...
        for lead in old_leads:
            totalFeedback = FeedBack.objects.filter(
                lead=lead).count()  # count no of feedbacks for a lead
            lead_dict = {'id': lead.id, 'personName': lead.personName, 'instituteName': lead.instituteName, 'contactPhone': lead.contactPhone, 'email': lead.email, 'numberStudents': lead.numberStudents,
                         'location': lead.location, 'notes': lead.notes, 'source': lead.source, 'assignedTo': lead.assignedTo.name, 'lead_status': lead.lead_status, 'totalFeedback': totalFeedback, 'totalFeedback': totalFeedback}
            leads_list.append(lead_dict)
...
class UpdateInstitute(APIView):
    def post(self, request):
        try:
            data = request.data
            institute = Institute.objects.get(id=data['id'])
